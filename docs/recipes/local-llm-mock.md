...
EOF
   ```
2. Start buddy with the preset name:
   ```bash
   buddy run local-llm
   ```
3. Inject a message through the mock transport (example using tests/scripts) or wire your own harness; nostr DMs are not used in mock mode.
